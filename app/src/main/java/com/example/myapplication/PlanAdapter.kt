...
class PlanAdapter(
    ...
    private val onAddSchedule: (PlanUiItem.DayHeader) -> Unit = {},
    private val onScheduleClick: (PlanUiItem.Schedule) -> Unit = {},
    private val onScheduleLongClick: (PlanUiItem.Schedule) -> Unit = {}
    ...
    inner class HeaderViewHolder(val binding: ItemDayHeaderBinding) : RecyclerView.ViewHolder(binding.root)
    inner class PlanViewHolder(val binding: ItemPlanBinding) : RecyclerView.ViewHolder(binding.root)

    override fun getItemViewType(position: Int): Int {
        return when(items[position]) {
            is PlanUiItem.DayHeader -> TYPE_HEADER
            is PlanUiItem.Schedule -> TYPE_PLAN
        }
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return if (viewType == TYPE_HEADER) {
            HeaderViewHolder(ItemDayHeaderBinding.inflate(LayoutInflater.from(parent.context), parent, false))
        } else {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when(val item = items[position]) {
            is PlanUiItem.DayHeader -> {
                ...
                headerHolder.binding.tvDayHeader.text = "Day ${item.day}"
                if (editable) {
                    headerHolder.binding.btnAddSchedule.visibility = View.VISIBLE
                    headerHolder.binding.btnAddSchedule.setOnClickListener {
                        // 위치 대신 객체 자체를 넘겨줌으로써 stale position 문제 방지
                        onAddSchedule(item)
                    }
                } else {
                    headerHolder.binding.btnAddSchedule.visibility = View.GONE
                }
            }
            ...
            is PlanUiItem.Schedule -> {
                ...
                planHolder.binding.tvTime.text = "$icon ${plan.time}"
                planHolder.binding.tvTitle.text = plan.title
                planHolder.binding.chipCategory.text = plan.category
                ...
                planHolder.binding.chipCategory.setChipBackgroundColor(
                    android.content.res.ColorStateList.valueOf(color)
                )

                holder.itemView.setOnClickListener { onScheduleClick(item) }
                holder.itemView.setOnLongClickListener {
                    onScheduleLongClick(item)
                    true
                }
            }
        }
